...
bool run_cmd (char* cmd_buf) {
	...
	//! clear screen
		...
		char *cpu_vendor = get_cpu_vender();
		DebugPuts("\nMarca CPU : ");
		DebugPuts(cpu_vendor);
		DebugPuts("\n");
	}
	...
	//! help
		...
		DebugPuts ("\nOS Development Series Demo");
		DebugPuts ("\nwcon una linea de comandos basica (CLI)\n\n");
		DebugPuts ("Comandos soportados:\n");
		DebugPuts (" - exit: se sale de la linea de comandos y para el sistema.\n");
		DebugPuts (" - cls: limpia la pantalla!\n");
		DebugPuts (" - cpuid: despliega marca de CPU\n");
		DebugPuts (" - help: despliega un mensaje\n");
	}
...
int _cdecl kmain (multiboot_info* bootinfo) {
	...
	DebugGotoXY (0,0);
	DebugPuts ("OSDev Series Demo");
	DebugPuts ("\nEscribe \"exit\" para salir, \"help\" para una lista de comandos\n");
	DebugPuts ("+-------------------------------------------------------+\n");

	run ();

	DebugPrintf ("\nComando exit recibido, ir a ciclo infinito.");
	sound(1000);
	for (;;);
	return 0;
}
